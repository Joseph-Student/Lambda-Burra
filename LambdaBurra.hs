...
winPlayerRound hu hl c cw m
    ...
    | sizeHand hl == 0 = putStrLn "              **Perdiste la Partida, Lambda se ha quedado sin cartas.**"
...
checkHand hm h p
    ...
    | (hm /= h) && (p == Lambda) = putStrLn "         Lambda no tiene cartas de la pinta de la mesa. Ha cargado del mazo"
    | otherwise = putStrLn ""
